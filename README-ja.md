...
- **実在するコンテナブロック**（チェスト・樽・シュルカーボックス・ホッパーなど）を開いたとき、指定リストのアイテムをホットバーへ自動補充
- チェスト付きトロッコ／ボート、エンダーチェスト、プラグイン製のメニュー（ショップ・仮想倉庫）では**意図的に動作しません**。クライアント側からは本物のコンテナと区別できず、ショップのボタンを誤って押すと実際に資産を失うため、確証が無い場合は動かさない方針にしています
- ON/OFF 切り替え可能（ホットキー対応）
