...
- **変更: 耐久値警告が、壊れかけのアイテムを使うたびに毎回出るようになりました**
  v1.2.2 までは、アイテムが警告状態に入った瞬間（メッセージ上は `2%` と表示）に 1 回だけ警告し、
  そのあといくら削っても鳴りませんでした。今後は残り耐久が 1% 以下のあいだ、耐久が 1 減るたびに
  毎回警告します。壊れかけのつるはしで掘れば 1 ブロックごとに警告が出ます。
  修繕などで耐久が**戻る**ときは、これまでどおり鳴りません。
- **同じ種類の無名の道具を 2 本持っていると、片方しか追跡されない問題を修正**
  もう 1 本を使っても警告が出ませんでした。スロットごとに独立して追跡するようにしました。
  その代わり、壊れかけのアイテムを別のスロットへ移すと 1 回だけ警告が出ます。
- 変更: 警告音は 1 tick に 1 回までにしました。壊れかけの防具一式で被弾しても、音が 4 重に
  重なりません。チャットには警告の出たアイテムがすべて表示されます
- サーバーを移ったとき警告の記録が残ったままになり、壊れかけの道具が次のサーバーで
  鳴らないことがある問題を修正
- いずれも Minecraft の全バージョン（1.17.1 〜 1.21.11）に同じように効きます
...
- 警告状態のあいだは、**そのアイテムを使うたび**（耐久が減るたび）に毎回警告
- 修繕などで耐久が**戻った**ときは鳴らない
- 同じ道具を 2 本以上持っていても、スロットごとに独立して警告
- ON/OFF 切り替え可能（ホットキー対応）
