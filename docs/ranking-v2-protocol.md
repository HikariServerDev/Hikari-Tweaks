...
受信ハンドラから例外を出してはいけない。MC 1.20.5 未満は Fabric の legacy
ハンドラが **netty のイベントループ上**で走るため、抜けた例外は
exceptionCaught に落ちて**クライアント切断**になる。1.20.5 以降は
クライアントスレッドで走るのでそのままクラッシュレポートになる。
v2 の受信は例外を捕まえてログに落とし、そのパケットを捨てるだけにすること。

v1（ranking_data / player_list_response）も同じ形に直してある。
パースは MC 非依存の `ScoreboardV1Codec` に切り出し、
サーバー由来の varint で `ArrayList` を先行確保しないよう初期容量を頭打ちにした
（20 億件を名乗られると約 7.6GB を確保しようとして OutOfMemoryError になっていた）。
