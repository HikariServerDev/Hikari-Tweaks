...
**耐久値警告が、壊れかけのアイテムを使うたびに毎回出るようになりました。** v1.2.2 までは、
残り耐久が 1% 以下になった瞬間（メッセージ上は `2%` と表示）に 1 回だけ警告し、そのあと
いくら削っても鳴りませんでした。今後は 1% 以下のあいだ、耐久が 1 減るたびに毎回警告します。
スロットごとに追跡するので同じ道具を 2 本持っていてもそれぞれ警告が出て、修繕で耐久が
戻るときは鳴りません。Minecraft の全バージョンに同じように効きます。
...
> 耐久アイテムの残り耐久が 1% 以下になったとき、チャット通知と効果音で警告します。手に持っているものだけでなく、インベントリの全スロットを確認します。警告状態に入っているあいだは、**そのアイテムを使うたび**（＝耐久が減るたび）に毎回警告します。修繕などで耐久が**戻った**ときは鳴りません。スロットごとに独立して追跡するので、同じ道具を 2 本以上持っていてもそれぞれ警告が出ます（道具を別のスロットへ移すと 1 回だけ余分に鳴ります）。効果音は 1 tick に 1 回までなので、防具一式が同時に警告状態でも音が重なりません。設定キーは `durabilityWarningEnabled`（**補助機能**タブ、既定 ON）。
<br><br>
